...
This command should be run using a database configuration DSN.`,
  ...
  $ matterfoss db init --config postgres://localhost/matterfoss

  # or you can use the MM_CONFIG environment variable
  $ MM_CONFIG=postgres://localhost/matterfoss matterfoss db init

  # and you can set a custom defaults file to be loaded into the database
  $ MM_CUSTOM_DEFAULTS_PATH=custom.json MM_CONFIG=postgres://localhost/matterfoss matterfoss db init`,
	Args: cobra.NoArgs,
	RunE: initDbCmdF,
}
