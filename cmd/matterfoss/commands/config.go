...
var ConfigSubpathCmd = &cobra.Command{
	...
	Long:  "Update the hard-coded production client asset paths to take into account Matterfoss running on a subpath.",
	Example: `  config subpath
  config subpath --path /matterfoss
  config subpath --path /`,
...
var MigrateConfigCmd = &cobra.Command{
	...
	Long:    "Migrate a file-based configuration to (or from) a database-based configuration. Point the Matterfoss server at the target configuration to start using it",
	Example: `config migrate path/to/config.json "postgres://mmuser:mostest@localhost:5432/matterfoss_test?sslmode=disable&connect_timeout=10"`,
