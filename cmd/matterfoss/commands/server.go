...
var serverCmd = &cobra.Command{
	Use:          "server",
	Short:        "Run the Matterfoss server",
	RunE:         serverCmdF,
	SilenceUsage: true,
}
...
func serverCmdF(command *cobra.Command, args []string) error {
	...
	if err := utils.TranslationsPreInit(); err != nil {
		return errors.Wrapf(err, "unable to load Matterfoss translation files")
	}
...
func runServer(configStore config.Store, disableConfigWatch bool, usedPlatform bool, interruptChan chan os.Signal) error {
	...
	if usedPlatform {
		mlog.Error("The platform binary has been deprecated, please switch to using the matterfoss binary.")
	}
