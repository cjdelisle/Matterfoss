...
func TestPluginConfigs(t *testing.T) {
	...
	err = UpdateMap(configMap, []string{"Plugins", "matterfoss-autolink", "enableadmincommand"}, []string{"true"})
	require.Nil(t, err, "Wasn't expecting an error")
	assert.Equal(t, true, configMap["Plugins"].(map[string]map[string]interface{})["matterfoss-autolink"]["enableadmincommand"].(bool))
