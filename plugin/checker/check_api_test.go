...
func TestCheckAPIVersionComments(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			pkgPath: "github.com/cjdelisle/matterfoss-server/v5/plugin/checker/internal/test/missing",
			err:     "could not find API interface",
		},
		{
			name:    "non-existent package path",
			pkgPath: "github.com/cjdelisle/matterfoss-server/v5/plugin/checker/internal/test/does_not_exist",
			err:     "could not find API interface",
		},
	}
