...
func (a *App) doPluginRequest(method, rawURL string, values url.Values, body []byte) (*http.Response, *model.AppError) {
	...
	r.Header.Set("Matterfoss-User-Id", a.Session().UserId)
...
func (a *App) buildWarnMetricMailtoLink(warnMetricId string, user *model.User) string {
	...
	mailBody += T("api.server.warn_metric.bot_response.mailto_diagnostic_id_header", map[string]interface{}{"DiagnosticId": "matterfoss"})
	mailBody += "\r\n"
	...
	mailToLinkContent := &MailToLinkContent{
		...
		MailRecipient: "support@matterfoss.org",
		MailCC:        user.Email,
