...
docker exec -i matterfoss-mysql mysql -D migrated -uroot -pmostest < $(pwd)/scripts/matterfoss-mysql-$SCHEMA_VERSION.sql

docker exec -i matterfoss-mysql mysql -D migrated -uroot -pmostest -e "INSERT INTO Systems (Name, Value) VALUES ('Version', '$SCHEMA_VERSION')"
...
if [ "$SCHEMA_VERSION" == "5.0.0" ]; then
  for i in "ChannelMembers SchemeGuest" "ChannelMembers MsgCountRoot" "ChannelMembers MentionCountRoot" "Channels TotalMsgCountRoot"; do
    ...
    docker exec matterfoss-mysql mysql -D migrated -uroot -pmostest -e "ALTER TABLE ${a[0]} DROP COLUMN ${a[1]};"
    docker exec matterfoss-mysql mysql -D latest -uroot -pmostest -e "ALTER TABLE ${a[0]} DROP COLUMN ${a[1]};"
  done
fi
